...
Model: system of 4 wells injection and extraction
"""
...
def _make_wel_stress_period(gwf, wel_qin, wel_qin_1, wel_qout, wel_qout_1, wel_coordsin, wel_coordsin_1, wel_coordsout,
                            wel_coordsout_1):
    """Create stress period data for the wel package."""
    period = flopy.mf6.ModflowGwfwel.stress_period_data.empty(
        gwf,
        maxbound=4, #integer value specifying the maximum number of wells cells that will be specified for use during any stress period.
    )
    period[0][0] = (wel_coordsin, wel_qin)
    period[0][1] = (wel_coordsout, wel_qout)
    period[0][2] = (wel_coordsin_1, wel_qin_1)
    period[0][3] = (wel_coordsout_1, wel_qout_1)
    return period
...
def make_input(
    ...
    flopy.mf6.ModflowGwfdis(gwf, nrow=100, ncol=100) # number ir rows and cols since the model has only one layer
...
def show_well_head(model_path, name, wel_coordsin, wel_coordsin_1, wel_coordsout, wel_coordsout_1):
    """Plot head at well over time."""
    sim = get_simulation(model_path, name)
    gwf = sim.get_model(name)
    heads_in = gwf.output.head().get_ts(wel_coordsin)
    heads_in_1 = gwf.output.head().get_ts(wel_coordsin_1)
    heads_out = gwf.output.head().get_ts(wel_coordsout)
    heads_out_1 = gwf.output.head().get_ts(wel_coordsout_1)
    _, ax = plt.subplots()
    ax.plot(heads_in[:, 0], heads_in_1[:, 0], heads_in[:, 1], heads_in_1[:, 1], heads_out[:, 0], heads_out_1[:, 0],
            heads_out[:, 1], heads_out_1[:, 1])

# pymf6 starting model with values as 0 to check if the pymf6 responds to the model
def do_all(model_path, name, wel_qin=0, wel_qin_1=0, wel_qout=0, wel_qout_1=0, verbosity_level=0):
    """Do all steps:
    ...
    * show head at well over time
    """
    wel_coordsin = (0, 2, 2)
    wel_coordsin_1 = (0, 4, 4)
    wel_coordsout = (0, 6, 6)
    wel_coordsout_1 = (0, 8, 8)
    make_input(
        wel_qin=wel_qin,
        wel_qin_1=wel_qin_1,
        wel_qout=wel_qout,
    ...
    show_well_head(model_path, name, wel_coordsin, wel_coordsin_1, wel_coordsout, wel_coordsout_1)
