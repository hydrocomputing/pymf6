...
def make_analytic_well_cells(wel, analytic_col_name='PYMF6_ANALYTIC'):
    ...
    if wel.get_advanced_var('inamedbound')[0]:
        bound_names = wel.get_advanced_var('boundname_idm')[:len(df)]
        df['wel_name'] = [name.lower() for name in bound_names]
...
def make_analytic_wells(gwf, analytic_well_data, kper):
    """Create new analytic  well models."""
    analytic_well_data = {} if analytic_well_data is None else analytic_well_data
    wel = gwf.wel
    analytic_well_cells = make_analytic_well_cells(wel)
    for name in analytic_well_data:
        if name not in analytic_well_cells:
            raise ValueError(f'no MF6 data for {name} in stress period {kper + 1}')
...
def run_model(model_path_controlled='models/c_100_100_multi_well', analytic_well_data=None):
    ...
    with warnings.catch_warnings(action='ignore'), open(devnull, 'w') as fobj:
        ...
        kper = gwf.kper
        res = {}
        analytic_well_cells, well_levels, awells = make_analytic_wells(gwf, analytic_well_data.get(kper), kper)
